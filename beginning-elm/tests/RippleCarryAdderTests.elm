module RippleCarryAdderTests exposing (rippleCarryAdderProprtiesTests, rippleCarryAdderTests)
...
--inverterTests =
--    describe "Inverter"
--        [ test "output is 0 whet the input is 1" <|
--            \_ ->
--                inverter 0
--                    |> Expect.equal 1
...
--                andGate 1 0
--                    |> Expect.equal 0
--        , test "output is 1 when the both inputs are 1" <|
--            \_ ->
--                andGate 1 1
--                    |> Expect.equal 1
--        ]
...
--            [ describe "and the second input is 0"
...
rippleCarryAdderProprtiesTests : Test
rippleCarryAdderProprtiesTests =
    describe "carry-out's relationship with significant digits"
        [ fuzz3
            (list (intRange 0 1))
            (list (intRange 0 1))
            (intRange 0 1)
          ...
          <|
            \list1 list2 carryIn ->
                let
                    ...
                    firstInput =
                        convertToBinary list1 + 1000

                    secondInput =
                        convertToBinary list2 + 1000
                in
                rippleCarryAdder firstInput secondInput carryIn
                    |> digits
                    |> List.length
          ...
          <|
            \list1 list2 carryIn ->
                let
                    ...
                    addOneToEnd digitsList =
                        List.append digitsList [ 1 ]
